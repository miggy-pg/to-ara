...
exports.addToFavorite = async(req, res) => {
  ...
  try {
    await db.query("INSERT INTO user_favorites (user_id, festival_id) VALUES ($1, $2);", [userId, id])
...
exports.removeFromFestivalFavorite = async(req, res) => {
  const { id, userId } = req.body

  try {
    await db.query("DELETE FROM user_favorites WHERE user_id = $1 AND festival_id = $2;", [userId, id])
  }
  catch(err){
    console.log(err)
  }
...
exports.getFestivalFavorites = async(req, res) => {
  try{
    const userId = req.params.userId
    const {rows} = await db.query("SELECT a.id, a.name, a.description, a.image FROM festival a JOIN user_favorites uf ON a.id = uf.festival_id WHERE uf.user_id = $1; ", [userId]);
